...
export default function JobManagement() {
  ...
  const stats = {
    ...
    applicants: jobs.reduce((count, job) => count + (job.applicant_count || 0), 0),
  };
  ...
  return (
    ...
    >
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <MetricCard helper="All roles created by your team." icon={BriefcaseBusiness} label="Total roles" value={stats.totalRoles} />
        <MetricCard helper="Listings currently available to candidates." icon={Sparkles} label="Active roles" tone="primary" value={stats.activeRoles} />
        <MetricCard helper="Roles not yet published." icon={Workflow} label="Drafts" tone="warning" value={stats.draftRoles} />
        <MetricCard helper="Combined inbound candidate volume." icon={Users} label="Applicants" tone="brass" value={stats.applicants} />
      </div>
          ...
          <div className="flex flex-wrap gap-2">
            {["all", "active", "closed", "draft"].map((status) => (
              <button
                ...
                type="button"
              >
                {status === "all" ? "All roles" : status[0].toUpperCase() + status.slice(1)}
              </button>
            ))}
          </div>
        </div>
      ...
      {filteredJobs.length === 0 ? (
        <EmptyPanel
          ...
          title={searchQuery || filterStatus !== "all" ? "No roles match these filters" : "No roles posted yet"}
        />
      ) : (
        <div className="grid gap-4">
          {filteredJobs.map((job) => (
            <SurfaceCard className="p-5 sm:p-6" key={job.job_id}>
              <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
                <div className="max-w-3xl">
                  <div className="flex flex-wrap items-center gap-2">
                    <StatusPill tone={statusTone(job.status)}>{job.status || "draft"}</StatusPill>
                    <StatusPill tone="info">{job.applicant_count || 0} applicants</StatusPill>
                  </div>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2">
                    <StatusPill>{job.employment_type || "Full Time"}</StatusPill>
                    <StatusPill>{job.posted_date ? format(new Date(job.posted_date), "MMM dd, yyyy") : "Date pending"}</StatusPill>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2 lg:justify-end">
                  ...
                  <Button onClick={() => navigate("/applications")} type="button" variant="outline">
                    View applicants
                  </Button>
                  {job.status === "draft" ? (
                    <Button onClick={() => handleDeleteJob(job.job_id)} type="button" variant="destructive">
                      Delete draft
                    </Button>
                  ) : (
                    <Button
                      ...
                      type="button"
                      variant={job.status === "active" ? "destructive" : "secondary"}
                    >
